            ...
            <div style="width: 100%;">
               <ul class="lnb_menu">
                  ...
                  <li>
                     ...
                     <!-- <input class="sub_button4" type="button" value="구매내역" onclick="getPurchaseList()"> -->
                  </li>
                  <li>
                     	<input class="sub_button5" type="button" value="작성글관리" onclick="getMyBoardList()">
                    </a>
                  </li>
               </ul>
            </div>
         ...
         <!-- 마이페이지 첫화면 회원정보 -->
            <div class="sub_button1">
                  <div class="sub_page">
                     ...
                     <form method="post" action="${pageContext.request.contextPath}/user/UserInfoChange.me" id="userInfoChangeOkForm">
                     <div>
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                                 <p id="user_nickname" style="text-align: left; padding: 0 1em; line-height: 3em;">${getuserInfo.getUser_nickname()}</p>
                              </div>
                           </fieldset>
                        </div>
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                                 <p style="text-align: left; padding: 0 1em; line-height: 3em;">${getuserInfo.getUser_name()}</p>
                              </div>
                           </fieldset>
                        </div>
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                              	<p style="text-align: left; padding: 0 1em; line-height: 3em;"><c:out value="${session_email}"/></p>
                              </div>
                           </fieldset>
                        </div>
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                                 <p style="text-align: left; padding: 0 1em; line-height: 3em;">${getuserInfo.getUser_tel()}</p>
                              </div>
                           </fieldset>
                        </div>
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                              	 <p style="text-align: left; padding: 0 1em; line-height: 3em;">(${getuserInfo.getUser_postcode()})</p>
                              	 <p style="text-align: left; padding: 0 1em; line-height: 3em;">${getuserInfo.getUser_address()}</p>
                              </div>
                           </fieldset>
                        </div>
                        <hr>
                           ...
                           <fieldset>
                              ...
                              <div id="info_content">
                                 <p style="width:100%; display:flex;">
                                    <input type="password" name="user_pw_original" style="float: left; width: 50%;" placeholder="현재 비밀번호">
                        ...
                        <div class="info_inner">
                           <fieldset>
                              ...
                              <div id="info_content">
                                 <p>
                                    <input type="password" name="user_pw" placeholder="새 비밀번호">
...
<script type="text/javascript">
      ...
      //#################################################################################
      //로그인 된 유저가 작성한 게시글 불러오기
		function getMyBoardList(){
        	 var user_nickname = $("#user_nickname").text();
        	 
        	 $.ajax({
        		 url: contextPath+"/board/MyBoardList.ub",
        		 type:"get",
        		 data: {"user_nickname" : user_nickname},
        		 dataType : "json",
        		 success: function(myBoards){
        			 showList(myBoards);
        		 },
         ...
         function showList(myBoards){
        	 var text = "";
        	 var num = 0;
        	 
        	 if(myBoards != null && myBoards.length !=0){
        		 	text += "<span style='font-size:smaller; margin: 0;'> 글 개수: " + myBoards.length + " 개</span>";
        			text += "<table>";
        			text += "<caption style='text-align:left; margin-bottom:3%;'></caption>";
        			text += "</caption>";
        			text += " <thead>";
        			text += "<tr id='trAlignCenter'>";
        			text += "<th style='width: 6%;'>번호</th>";
        			text += "<th style='width: 30%;'>제목</th>";
        			text += "<th style='width: 10%;'>작성자</th>";
        			text += "<th style='width: 12%;'>날짜</th>";
        			text += "<th style='width: 6%;'>조회수</th>";
        			text += "<th style='width: 6%;'>추천수</th>";
        			text += "</tr>";
        			text += "</thead>";
        			text += "<tbody>";
        			
        		 $.each(myBoards, function(index, board){
        		 	text += "<tr id='scriptTable'>";
        		 	text += "<td>" + ++num + "</td>";
        		 	text += "<td style='text-align: left;'><a href='${pageContext.request.contextPath}/board/UserBoardViewOk.ub?user_board_num=${userBoard.getUser_board_num()}&user_nickname=${userBoard.getUser_nickname()}&nickname=${nickname}'>" + board.user_board_title + "</a></td>";
        		 	text += "<td>" + board.user_nickname + "</td>";
        		 	text += "<td>" + board.user_board_date + "</td>";
        		 	text += "<td>" + board.user_board_views + "</td>";
        		 	text += "<td>" + board.user_board_likes + "</td>";
        		 	text += "</tr>";
        		 });
        		 	text += "</tbody>";
        		 	text += "</table>";
        	 }else{
        		 text = "<p>작성하신 게시글이 없습니다.</p>";
        	 }
        	 $("#myBoards").html(text);
      ...
      //#################################################################################
      //로그인 된 유저가 구매한 리스트 불러오기
		function getPurchaseList(){
    	  
	   	 $.ajax({
	   		 url: contextPath+"/shopping/PurchaseListOk.sh",
	   		 type:"get",
	   		 dataType : "json",
	   		 success: function(myPurchases){
	   			 showPurchase(myPurchases);
	   		 },
	   		 error:function(request,status,error){
	   			 console.log("오류");
	   		 }
      	...
      	function showPurchase(myPurchases){
	       	 var text = "";
	    	 
	    	 if(myPurchases != null && myPurchases.length !=0){
	    		 text += "<table>";
	    		 text += "<thead>";
	    		 text += "<tr>";
	    		 text += "<th style='width:16%;'>이미지</th>";
	    		 text += "<th style='width:34%;'>상품명</th>";
	    		 text += "<th style='width:18%;'>주문날짜</th>";
	    		 text += "<th style='width:18%;'>주문번호</th>";
	    		 text += "<th style='width:14%;'>금액</th>";
	    		 text += "</tr>";
	    		 text += "</thead>";
	    		 text += "<tbody>";
	    		 
	    		 $.each(myPurchases, function(index, purchase){
		    		 text += "<tr>";
		    		 text += "<td style='border-left:none; text-align:center;'>";
		    		 text += "<div>";
		    		 text += "<img src=${pageContext.request.contextPath}/images/" + purchase.shopitem_file_name + " class='item'>";
		    		 text += "</div>";
		    		 text += "</td>";
		    		 text += "<td>";
		    		 text += "<div class='purchasecontent'>" + purchase.shopitem_name + "</div>";
		    		 text += "<div class='purchasecontent'><strong>[수량 : " + purchase.item_cnt + "개]</strong></div>";
		    		 text += "</td>";
		    		 text += "<td><div class='purchasecontent'>" + purchase.purchase_date + "</div></td>";
		    		 text += "<td><div class='purchasecontent'>" + purchase.purchase_num + "</div></td>";
		    		 text += "<td style='border-right:none;'><div class='purchasecontent'>" + purchase.purchase_cost + "원</div></td>";
		    		 text += "</tr>";
	    		 });
		    		 text += "</tbody>";
		    		 text += "</table>";
	    	 	 }else{
	    			 text = "<p>구매하신 목록이 없습니다.</p>";
	    		 }
				 $("#myPurchase").html(text);
      ...
      function formSubmit_changeInfo(){
         ...
         if(!$("input[name='user_pw_original']").val()){
            alert("현재 비밀번호를 입력해주세요");
            $("input[name='user_pw_original']").focus();
            return;
         }
         
         //현재 비밀번호 입력 후 아무런 정보기입도 없이 정보수정을 누를 경우
         if(!$("input[name='user_pw']").val() && !$("input[name='password_re']").val() &&/*  !$('input#user_tel1').val() && */ !$("input[name='user_postcode']").val()){
            alert("수정하실 정보를 입력해주세요");
            return;
         }
         
         //새비밀번호 입력없이, 새비밀번호 확인만 한 경우
         if(!$("input[name='user_pw']").val() && $("input[name='password_re']").val()){
            alert("새비밀번호를 입력해주세요");
		 ...
		 if($("input[name='user_pw_original']").prop("readonly") == false && $("input[name='user_pw']").prop("readonly") == false && $("input[name='password_re']").prop("readonly") == false){
       ...
       //#################################################################################
        ...
        function checkPw(){
           ...
           $("input[name='user_pw_original']").on("click", function(){
        	   $("#pwCheck_text").text("");
           });
			
           var user_pw = $("input[name='user_pw_original']").val();
           ...
           $.ajax({
              ...
              success:function(result){
                 if(result.status == "ok"){
                    ...
                    $("input[name='user_pw_original']").prop("readonly", true);
                 }else{
                    //DOM
      ...
      //#################################################################################
       ...
       $("input[name='user_pw']").on("click", function(){
           $("input[name='user_pw']").prop("readonly", false);
       });
       ...
       $("input[name='user_pw']").keyup(function(){
          ...
          $("input[name='password_re']").prop("readonly", false);
          $("#pwCheck_re_text").text("");
         
         if($("input[name='user_pw']").val()){
            var pwTag = $("input[name='user_pw']");
            var pw = pwTag.val();
            ...
            if(pw.search(/\s/) != -1){
               ...
               $("input[name='user_pw']").prop("readonly", true);
            }
      }
         });
       
      //새비밀번호 확인 입력후 창에서 벗어나면 checkPw_re() 실행
       $("input[name='password_re']").on("click", function(){
           $("input[name='password_re']").prop("readonly", false);
       });
