...
<c:set var="area" value="${area}"/>
	...
	<section id="marketGallery" class="wrapper"
		...
		<div>
			<input type="button" id="famous" value="배달가능" onclick="getKategorieList(this)"/>
		</div>
		<div>
	...
	<section id="marketGallery" class="wrapper"
		...
		<c:forEach var="mart" items="${marts[0]}">
			<div id="marketPost">
			<a href="${pageContext.request.contextPath}/martsearch/MartInfoOK.ma?mart_num=${mart.getMart_num()}">
...
<script>
	function getAreaList(val) {
		var area = val.value;
		location.href="${pageContext.request.contextPath}/martsearch/AreaOK.ma?area="+area;
	}
	
	function getKategorieList(val) {
		var kategorie = val.value;
		location.href="${pageContext.request.contextPath}/martsearch/KategorieOK.ma?area=${area}&kategorie="+kategorie;
	}
